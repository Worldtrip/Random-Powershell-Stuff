...
[array] $Events = @()
$Events += Get-EventLog -ComputerName $ComputerName -LogName Application -After $StartTime `
			| where { ($IncludeEntryTypes -contains $_.EntryType) }
$StopWatch.Stop()			
Write-Host $ComputerName ":  Get-EventLog on Application log found" `
	$Events.Count "events. Elapsed time:" $StopWatch.Elapsed
...
$StopWatch.Stop()			
Write-Host $ComputerName ":  Get-WinEvent on Application log found" `
	$Events.Count "events. Elapsed time:" $StopWatch.Elapsed
...
[array] $Events = @()
$Events += Get-EventLog -ComputerName $ComputerName -LogName Security -After $StartTime `
			| where { ($IncludeEntryTypes -contains $_.EntryType) }
$StopWatch.Stop()			
Write-Host $ComputerName ":  Get-EventLog on Security log found" `
	$Events.Count "events. Elapsed time:" $StopWatch.Elapsed
...
$Events += Get-WinEvent -ComputerName $ComputerName -FilterXml $XMLfilter
$StopWatch.Stop()			
Write-Host $ComputerName ":  Get-WinEvent on Security log found" `
	$Events.Count "events. Elapsed time:" $StopWatch.Elapsed
